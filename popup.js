...
const saveApiSettings = async () => {
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      chrome.runtime.sendMessage({ action: 'setApiEndpoint', apiEndpoint: finalEndpoint }, (response) => {
        if (chrome.runtime.lastError) {
          console.error('Error saving API endpoint:', chrome.runtime.lastError);
          reject(new Error(chrome.runtime.lastError.message));
        } else {
          console.log('API endpoint saved successfully:', response);
          resolve(response || { success: true });
        }
      });
    });
    ...
    // 🛑 CRITICAL DEBUGGING STEP: Capture the specific Chrome Runtime Error
    const runtimeError = chrome.runtime.lastError ? ` (Runtime Error: ${chrome.runtime.lastError.message})` : '';
    ...
    console.error('Error saving API settings:', error, runtimeError);
    
    // Display a clear error message to the user, including the technical runtime detail
    showError(currentLanguage === 'ar' 
      ? `خطأ حرج في حفظ الإعدادات. تحقق من عمل ملف الخلفية (Background): ${error.message}${runtimeError}` 
      : `Critical error saving settings. Check Background Script functionality: ${error.message}${runtimeError}`);
...
const updateUIText = (language) => {
  ...
  if (logoApiScreen) {
    // Fix: Use chrome.runtime.getURL for proper image path in Chrome Extension
    try {
...
const setupEventListeners = () => {
  ...
  // ========== Export Buttons ==========
  // FIX: Export buttons are handled in setupEventListeners() below
  // This section removed to avoid duplicate handlers
    ...
    navTabs.forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        if (targetTab === 'evaluation') {
          ...
          // Initialize training section if not already done
          if (typeof initTrainingSection === 'function') {
            initTrainingSection();
          }
        }
...
const init = async () => {
  ...
  // Initialize environment select element
  ...
  // Load logo images using chrome.runtime.getURL
  const logoApiScreen = document.getElementById('logo-api-screen');
  const logoMainScreen = document.getElementById('logo-main-screen');
  ...
  // PDF export button removed - no longer needed
  ...
  if (exportWordBtn) {
    exportWordBtn.addEventListener('click', async () => {
      if (currentResults.length > 0) {
        ...
        // FIX: Format recommendations using formatRecommendations to match what's shown on screen
  ...
  // Initialize training section (only if main screen is available)
  if (mainScreen) {
...
const initTrainingSection = () => {
  ...
  if (navTabs && navTabs.length > 0) {
    ...
    console.warn('Navigation tabs not found. Elements may not be loaded yet.');
    // Try again after a delay
    setTimeout(() => {
      navTabs = document.querySelectorAll('.nav-tab');
      if (navTabs && navTabs.length > 0) {
        console.log('Retrying tab setup...');
        initTrainingSection();
      }
    }, 500);
  }

  // Setup close viewer button
  if (closeViewerBtn) {
...
const switchTab = (tabName) => {
  ...
  if (tabName === 'evaluation') {
    if (evaluationTab) evaluationTab.classList.add('active');
    if (trainingTab) trainingTab.classList.remove('active');
  } else if (tabName === 'training') {
    if (evaluationTab) evaluationTab.classList.remove('active');
...
const loadFileAsImage = (file) => {
  ...
  try {
    ...
    // Always use chrome.runtime.getURL for extension resources
    if (chrome && chrome.runtime && chrome.runtime.getURL) {
      try {
        imageUrl = chrome.runtime.getURL(imagePath);
        console.log('Loading image from:', imageUrl, 'Original path:', imagePath);
      } catch (e) {
        console.warn('Could not use chrome.runtime.getURL:', e);
        // Fallback: try relative path
        imageUrl = imagePath;
      }
    } else {
      console.warn('chrome.runtime.getURL not available, using direct path');
      imageUrl = imagePath;
    }
